...
def suma_resta_alternada(lista: list[int]) -> int:
    def operacion_interna(lista: list[int], suma: int, signo: int) -> int:
        ...
        if not lista:
            ...
            return operacion_interna(lista=lista[1:], suma=suma + signo * lista[0], signo=-signo)
    
    return operacion_interna(lista=lista, suma=0, signo=1)


if __name__ == '__main__':
    lista = [1, 2, 3]
    print(suma_resta_alternada(lista)) # 2
